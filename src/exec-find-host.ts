...
export async function main(ns: NS) {
    ...
    const shouldExcludeHost = (host: string) => !/remote-server|home|darkweb/.test(host)
    const connectedHosts = (host: string) => ns.scan(host).filter(shouldExcludeHost)
    const visitedHosts: string[] = [];

    const findHost = ({
        path = [],
        baseHost
    }: { path?: string[], baseHost: string }): string[] => {
